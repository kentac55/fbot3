...
export const registerEventHandlers = (
  ...
  rtm.on(EventKind.Authenticated, (ev: models.ConnectedEvent): void => {
    ...
    getVersion()
      ...
      .catch((e): void => {
        _log.error(e)
        const msg: ChatPostMessageArguments = {
          text: `:information_source: ${ev.self.name}(status: :thinking_face:)`,
          ...msgBase,
        }
  ...
  rtm.on(EventKind.EmojiChanged, (ev: models.EmojiEvent): void => {
    ...
    if (ev.subtype === EmojiEventKind.Add) {
      ...
      web.chat
        .postMessage(msg)
        .then((result: WebAPICallResult): void => {
          _log.info(result)
        })
        .catch((e: Error): void => {
          _log.error(e)
      ...
      ev.names.forEach((name: string): void => {
        const msg: ChatPostMessageArguments = {
  ...
  rtm.on(EventKind.ChannelArchive, (ev: models.ChannelArchiveEvent): void => {
      ...
      .then(
        (
          ...
          if (isValidTuple(results)) {
            ...
            const msg: ChatPostMessageArguments = {
              text: `:skull: channel(#${channel.channel.name}) has been archived by @${user.user.name}`,
              ...msgBase,
            }
            return web.chat.postMessage(msg)
  ...
  rtm.on(EventKind.ChannelCreated, (ev: models.ChannelCreatedEvent): void => {
      ...
      .then(
        (user: WebAPICallResult): Promise<WebAPICallResult> => {
          if (isUserInfoResult(user)) {
            const msg: ChatPostMessageArguments = {
              text: `:baby: channel created. #${ev.channel.name} by @${user.user.name}`,
              ...msgBase,
            }
            return web.chat.postMessage(msg)
          } else {
  ...
  rtm.on(EventKind.ChannelDeleted, (ev: models.ChannelDeleted): void => {
    const _log = log.child({ event: EventKind.ChannelDeleted })
    const msg: ChatPostMessageArguments = {
      text: `:cop: channel(id: ${ev.channel}) has been *deleted* by *admin*`,
      ...msgBase,
    }
    web.chat
      .postMessage(msg)
  ...
  rtm.on(EventKind.ChannelRename, (ev: models.ChannelRenameEvent): void => {
    const _log = log.child({ event: EventKind.ChannelRename })
    const msg: ChatPostMessageArguments = {
      text: `:writing_hand: a channel has been renamed to #${ev.channel.name}`,
      ...msgBase,
    }
    web.chat
      .postMessage(msg)
        ...
        .then(
          (
            ...
            if (isValidTuple(results)) {
              const [channel, user] = results
              const msg: ChatPostMessageArguments = {
                text: `:zombie: channel(#${channel.channel.name}) has been unarchived by @${user.user.name}`,
                ...msgBase,
              }
              return web.chat.postMessage(msg)
            } else if (results[0].ok) {
  ...
  rtm.on(EventKind.Message, (ev: models.MessageEvent): void => {
        ...
        .then(
          (results): Promise<WebAPICallResult> => {
            if (isUserInfoResult(results[0]) && isUserInfoResult(results[1])) {
              ...
              const msg: ChatPostMessageArguments = {
                text: `:loudspeaker: @${creator.user.name} 抜け忍を検知 => @${target.user.name}`,
                channel: ev.channel,
                as_user: true,
                link_names: true,
              }
  ...
  rtm.on(EventKind.FileCreated, (ev: models.FileCreated): void => {
